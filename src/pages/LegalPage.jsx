...
export default function LegalPage({ type }) {
    ...
    return <main className="min-h-screen bg-[#f2efe6] text-[#15171b] p-6 md:p-12"><div className="max-w-[760px] mx-auto"><Link to="/login" className="font-semibold text-[#8a5717] underline underline-offset-2">EduShift’e dön</Link><header className="border-b-2 border-[#15171b] pb-6 mt-14"><p className="font-mono text-[10px] uppercase tracking-[.14em] text-[#6b6659] mb-3">EduShift / okul operasyon merkezi</p><h1 className="text-4xl md:text-5xl font-semibold">{page.title}</h1><p className="text-[#6b6659] leading-relaxed mt-5 max-w-xl">{page.intro}</p></header><div className="divide-y divide-[#e3ddd0]">{page.sections.map(([title, text], index) => <section key={title} className="py-7 grid md:grid-cols-[150px_1fr] gap-5"><p className="font-mono text-xs text-[#6b6659]">0{index + 1} / {title}</p><p className="text-sm leading-7 max-w-xl">{text}</p></section>)}</div><footer className="pt-6 border-t border-[#e3ddd0] text-xs text-[#6b6659]">Son güncelleme: 20 Eylül 2026</footer></div></main>
}